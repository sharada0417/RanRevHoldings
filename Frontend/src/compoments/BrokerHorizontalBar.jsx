...
const BrokerHorizontalBar = () => {
  const menuItems = [
    { label: "Brokers", to: "/broker" },
    { label: "View Brokers", to: "/broker/viewbrokers" },
  ];
        ...
        <nav className="flex items-center justify-evenly px-3 py-3 sm:px-6 sm:py-6">
          {menuItems.map((item) => (
            <NavLink
              key={item.to}
              to={item.to}
              end={item.to === "/broker"} // ✅ only the main tab ends
              className={({ isActive }) =>
...
export default BrokerHorizontalBar;
